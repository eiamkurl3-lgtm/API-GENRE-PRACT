...
description: Use when the user asks to create a new entity from scratch. Builds the full stack: entity, repository with stored procedures, DTOs, AutoMapper mappings, CRUD endpoints, and DI registration. Orchestrates the automapper-coder and stored-procedure-coder patterns internally.
---
...
### Step 7: Add Mappings to AutoMapperProfiles

All mappings go in a single file: `Utilities/AutoMapperProfiles.cs`. Check if the file exists, then add the new entity's mappings.
...
If the file already exists, add the new entity's mappings inside the constructor:
...
**Rules:**
- All mappings go in `Utilities/AutoMapperProfiles.cs` — no individual profile files
- Must include both mappings per entity:
  - `Create{Entity}DTO -> {Entity}` (for Create/Update requests)
  - `{Entity} -> {Entity}DTO` (for responses)
- Add new mappings inside the existing constructor, don't overwrite existing ones
...
- `Utilities/AutoMapperProfiles.cs` has both mappings for the new entity
...
5. **Profile convention** — All mappings go in `Utilities/AutoMapperProfiles.cs`. No individual profile files. Must include both mappings per entity.
